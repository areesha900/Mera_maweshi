...
import { PAKISTAN_DATA } from './pakistanData';
...
export default function RegistrationScreen() {
  ...
  const ucOptions: string[] = tehKey
    ? (() => {
        const data = PAKISTAN_DATA[provKey].districts[distKey].tehsils[tehKey];
        return isUrdu ? data.ucs_ur : data.ucs;
      })()
    : [];

  // Cascade handlers
  const handleProvince = (val: string) => { setProvince(val); setDistrict(''); setTehsil(''); setUc(''); setOpenDropdown(null); };
  const handleDistrict = (val: string) => { setDistrict(val); setTehsil(''); setUc(''); setOpenDropdown(null); };
  const handleTehsil   = (val: string) => { setTehsil(val);   setUc('');     setOpenDropdown(null); };
  const handleUc       = (val: string) => { setUc(val);        setOpenDropdown(null); };
  ...
  const t = {
    ...
    uc:       isUrdu ? 'یونین کونسل *'          : 'Union Council *',
    ...
    selUc:    isUrdu ? 'پہلے تحصیل منتخب کریں'  : 'Select Tehsil first',
      ...
      >
        <View style={styles.card}>
          ...
          <CascadePicker
            ...
            options={ucOptions}
            onSelect={handleUc}
            placeholder={tehsil ? 'Select Union Council' : t.selUc}
            disabled={!tehsil}
          />
          ...
          <TouchableOpacity
            style={styles.btn}
            onPress={() => {
              if (!name.trim() || !province || !district || !tehsil || !uc) {
                setShowError(true);
                return;
              }
              setShowError(false);
